...
export default function RecurringSummary({ recurring }) {
  if (recurring.length === 0) return null;

  // Group by description (rough grouping)
  const groups = {};
  recurring.forEach(t => {
    const key = t.description.toLowerCase().substring(0, 25);
    if (!groups[key]) {
      groups[key] = { description: t.description, category: t.category, total: 0, count: 0 };
    }
  ...
  const totalMonthly = entries.reduce((s, e) => s + (e.total / Math.max(e.count, 1)), 0);
  const totalAnnual = totalMonthly * 12;

  return (
    <div className="rounded-xl p-5" style={{ background: "#151a22", border: "1px solid rgba(255,255,255,0.06)" }}>
      <h3 className="text-sm font-semibold mb-4" style={{ color: "#f1f5f9" }}>Subscriptions & Recurring</h3>
        ...
        {entries.slice(0, 10).map((e, i) => (
          <div key={i} className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium whitespace-nowrap" style={{ color: "#f1f5f9" }}>
              {formatEUR(e.total / Math.max(e.count, 1))}/mo
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="text-sm" style={{ color: "#94a3b8" }}>
          Total: <span className="font-bold" style={{ color: "#f472b6" }}>{formatEUR(totalMonthly)}/month</span>
          <span className="ml-2">({formatEUR(totalAnnual)}/year)</span>
        </div>
      </div>
    </div>
  );
