...
export default function TopGastos({ expenses }) {
  const sorted = [...expenses]
    .filter(t => t.category !== "Traspaso Interno")
  ...
  return (
    <div className="rounded-xl p-5" style={{ background: "#151a22", border: "1px solid rgba(255,255,255,0.06)" }}>
      <h3 className="text-sm font-semibold mb-4" style={{ color: "#f1f5f9" }}>🔥 Top 10 Gastos Más Grandes</h3>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr>
              {["#", "Fecha", "Descripción", "Categoría", "Importe"].map(h => (
                <th key={h} className="text-left px-2 py-1.5 text-xs font-medium" style={{ color: "#64748b" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
